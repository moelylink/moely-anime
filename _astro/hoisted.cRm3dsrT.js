    ...
    `;try{const E=(await Promise.allSettled(r.map(c=>c.data()))).filter(c=>c.status==="fulfilled").map(c=>{const p=c.value.meta;return{id:parseInt(p.id),title:p.title,orig:p.orig,cover:p.cover,score:parseFloat(p.score||"0"),date:p.date,studio:p.studio,tags:p.tags?p.tags.split(","):[]}});g.innerHTML=E.map(c=>`
        <a href="/bangumi/${c.id}/" class="anime-card glass glass-hover">
          ...
          <div class="card-info">
            ...
            <div class="card-tags">
              ${c.tags.slice(0,3).map(k=>`<span class="tag-pill">${k}</span>`).join("")}
            </div>
          </div>
        </a>
      `).join(""),x.style.display=a>1?"flex":"none",G(a)}catch(d){console.error("Failed to render Pagefind results:",d),g.innerHTML=`
        <div class="empty-state glass">
          <span class="empty-icon"><i class="fa-solid fa-triangle-exclamation"></i></span>
          <p class="empty-title">加载页面数据失败</p>
        </div>
      `}}async function I(){await F()}function G(e){B.classList.toggle("disabled",n===1),M.classList.toggle("disabled",n===e);let a="";const s=5;let i=Math.max(1,n-2),r=Math.min(e,i+s-1);r-i<s-1&&(i=Math.max(1,r-s+1)),i>1&&(a+='<button class="page-num" data-page="1">1</button>',i>2&&(a+='<span class="page-dots">...</span>'));for(let d=i;d<=r;d++)a+=`
        <button class="page-num ${d===n?"active":""}" data-page="${d}">
          ${d}
        </button>
      `;r<e&&(r<e-1&&(a+='<span class="page-dots">...</span>'),a+=`<button class="page-num" data-page="${e}">${e}</button>`),q.innerHTML=a,document.querySelectorAll(".page-num").forEach(d=>{d.addEventListener("click",async E=>{const c=E.target;n=parseInt(c.dataset.page),await I(),window.scrollTo({top:g.offsetTop-120,behavior:"smooth"})})})}A();
