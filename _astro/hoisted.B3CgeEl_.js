import"./hoisted.CEIXwU9N.js";let L=null,f=null,n=1;const h=40,t={search:"",score:"all",year:"all",tag:"all",sort:"rank"},S=document.getElementById("search-wrapper"),b=document.getElementById("search-tags"),o=document.getElementById("search-input"),v=document.getElementById("search-clear-btn"),T=document.getElementById("sort-select"),u=document.getElementById("score-select"),y=document.getElementById("year-select"),k=document.getElementById("results-count"),d=document.getElementById("anime-grid"),P=document.getElementById("prev-page"),B=document.getElementById("next-page"),C=document.getElementById("page-numbers"),x=document.getElementById("pagination-controls");async function H(){try{L=await new Function('return import("/pagefind/pagefind.js")')(),console.log("Pagefind static index loaded successfully."),await L.options({ranking:{metaWeights:{title:5,orig:4,director:3,studio:3,tags:2}}})}catch{console.warn("Pagefind not available."),d.innerHTML=`
    ...
    `;try{const w=(await Promise.allSettled(i.map(c=>c.data()))).filter(c=>c.status==="fulfilled").map(c=>{const g=c.value.meta;return{id:parseInt(g.id),title:g.title,orig:g.orig,cover:g.cover,score:parseFloat(g.score||"0"),date:g.date,studio:g.studio,tags:g.tags?g.tags.split(","):[]}});d.innerHTML=w.map(c=>`
        <a href="/bangumi/${c.id}/" class="anime-card glass glass-hover">
          ...
          <div class="card-info">
            ...
            <div class="card-tags">
              ${c.tags.slice(0,3).map($=>`<span class="tag-pill">${$}</span>`).join("")}
            </div>
          </div>
        </a>
      `).join(""),x.style.display=a>1?"flex":"none",_(a)}catch(r){console.error("Failed to render Pagefind results:",r),d.innerHTML=`
        <div class="empty-state glass">
          <span class="empty-icon"><i class="fa-solid fa-triangle-exclamation"></i></span>
          <p class="empty-title">加载页面数据失败</p>
        </div>
      `}}async function I(){await M()}function _(e){P.classList.toggle("disabled",n===1),B.classList.toggle("disabled",n===e);let a="";const s=5;let l=Math.max(1,n-2),i=Math.min(e,l+s-1);i-l<s-1&&(l=Math.max(1,i-s+1)),l>1&&(a+='<button class="page-num" data-page="1">1</button>',l>2&&(a+='<span class="page-dots">...</span>'));for(let r=l;r<=i;r++)a+=`
        <button class="page-num ${r===n?"active":""}" data-page="${r}">
          ${r}
        </button>
      `;i<e&&(i<e-1&&(a+='<span class="page-dots">...</span>'),a+=`<button class="page-num" data-page="${e}">${e}</button>`),C.innerHTML=a,document.querySelectorAll(".page-num").forEach(r=>{r.addEventListener("click",async w=>{const c=w.target;n=parseInt(c.dataset.page),await I(),window.scrollTo({top:d.offsetTop-120,behavior:"smooth"})})})}H();
